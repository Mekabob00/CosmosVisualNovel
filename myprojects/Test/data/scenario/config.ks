...
;	メニューボタン非表示
	...
	tf.config_x       = [1560, 600, 681, 762, 843, 924, 1005, 1086, 1167, 1248, 1329]; // X座標（共通）

	tf.config_y_bgm   = 285; // BGMのY座標
	tf.config_y_se    = 375; // SEのY座標
	tf.config_y_ch    = 485; // テキスト速度のY座標
	tf.config_y_auto  = 575; // オート速度のY座標
...
;	画面右上の「Back」ボタン
	[button fix="true" width=130 height=130 graphic="&tf.img_path + 'c_btn_back.png'" enterimg="&tf.img_path + 'c_btn_back2.png'" target="*backtitle" x="1760" y="20"]
...
;	未読スキップ-OFF
	[button name="unread_off" fix="true" target="*skip_off" graphic="&tf.btn_path_off" width="255" height="68" x="600" y="705"]

;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="255" height="68" x="870" y="705"]
...
;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="100" top="820" width="1760" height="200" margint="2" marginl="30" page="fore" visible="true" opacity="0"]
